...
\begin{theorem}\label{thm:two}\ \leanmixed{Theorem2.lean::two\_noHalt\_full,\ bstar\_cppbe\_iff\_coe,\ two\_unique}{BetweennessPending.lean::two\_generic\_noHalt\_full,\ generic\_cppbe\_iff\_coe,\ two\_B\_generic\_unique}
...
\begin{definition}\label{dfn:thinB}\ \leanpending{ThinB.lean::clusterValues,\ thinB;\ ThinBExistence.lean::DisclosureGame.IsThinB}
...
The adjective ``thin'' refers to the primitive graph before convexification: away from discontinuities the correspondence is the singleton $\{v(\mu)\}$, while at a discontinuity it contains precisely the interval spanned by limit values. The following result verifies that this construction is compatible with the standing model.

\begin{lemma}\label{lem:thinB-A4}\ \leanpending{ThinB.lean::thinB\_A4}
...
The next proposition is the key property that distinguishes thin-B from an arbitrary interval-valued correspondence. If several beliefs share a feasible payoff, the value at every strict mixture of those beliefs is exactly the intersection of their values.
...
\begin{lemma}\label{lem:thinB-value-set}\ \leanpending{ThinBExistence.lean::thinB\_coalition\_payoff\_set}
...
The equality in \cref{lem:thinB-value-set} is stronger than the corresponding single-valued identity \cref{lem:value-id}. It permits any coalition strategy to be repaid at every point of the interval $V(\mu^0_C)$. This flexibility lets us normalize genuine PBE partitions without replacing the correspondence $V$ by the generally invalid single-valued selection $\{\overline v\}$.

\begin{proposition}\label{prop:thinB-upper-pbe}\ \leanpending{ThinBExistence.lean::IsThinB.hasCommonValueIntersections,\ exists\_upperNormalizedPBE}
    Every disclosure game obtained by restricting a thin-B correspondence to a non-empty face of $\Delta\Theta$, and possibly deleting messages while leaving every type with a non-empty message set, admits a PBE partition
    \[
...
\begin{theorem}\label{thm:thinB-cppbe}\ \leanpending{ThinBExistence.lean::thinB\_existence\_of\_continuousUpper}
    If $V$ is thin-B and its upper envelope $\overline v$ is continuous, then $G$ admits a coalition-proof PBE.
\end{theorem}

\begin{conjecture}\label{conj:thinB-cppbe}\ \leanpending{ThinBExistence.lean::thinB\_existence}
    Every game with a thin-B payoff correspondence admits a coalition-proof PBE.
\end{conjecture}

\Cref{thm:two-B,thm:thinB-cppbe} give, respectively, the single-valued result and the thin-B result under a continuous upper envelope. Under single-valued B, $V=\{v\}$ and continuity follows from \cref{A4:V-uhc}; under thin-B, the generator may be discontinuous and $V$ records its limiting values. The argument does not establish \cref{conj:thinB-cppbe}. In particular, it cannot simply apply the single-valued theorem to $\overline v$: although $\overline v$ satisfies B, it is generally only upper semicontinuous, and the singleton correspondence $\{\overline v\}$ need not satisfy \cref{A4:V-uhc}. More specifically, the proof of the level-set order in \cref{lem:btw-order} uses continuity in its two-sided separation case. Upper semicontinuity is insufficient: on the unit interval, the B function $v(x)=0$ for $x<1$ and $v(1)=1$ is already its own upper-semicontinuous envelope, and level-$0$ points converge to the level-$1$ endpoint. Thus a proof of \cref{conj:thinB-cppbe} requires either a level-set ranking theorem adapted to this boundary-jump geometry or a different selection argument. The normalization result \cref{prop:thinB-upper-pbe} supplies the correspondence-specific part of either route.
...
The proof of \cref{thm:two-B} rests on the following ordering of each level set of $v$.

\begin{lemma}\label{lem:btw-order}\ \leanproved{BetweennessOrder.lean::btw\_order}
    Assume B and fix $y\in\mathbb{R}$. Write $L:=\{\mu\in\Delta\Theta\sep v(\mu)=y\}$, $L^{++}:=\{v>y\}$, and $L^{--}:=\{v<y\}$. There is a complete and transitive relation $\succeq$ on $L$ such that:
    \begin{enumerate}
...
We now check that every relevant slice is lower-dimensional. This is the only place where semicontinuity is used. Because $v$ is upper semicontinuous, $L^{--}$ is relatively open in $\Delta\Theta$; since $B_F$ is non-empty, $\aff(B_F)=\aff(F)$. Fix $\lambda$ with $F_\lambda\cap S_F\neq\varnothing$. If $h_\lambda$ were constant on $F$, then evaluating it at a point of $F_\lambda$ would make the constant zero. For $\lambda\in(0,1)$, the equality $\lambda a+(1-\lambda)b=0$ on $B_F$, together with $a\leq0$ and $b\leq0$ there, would force $a=b=0$ on $B_F$. Since $\aff(B_F)=\aff(F)$, $b$ would vanish on all of $\aff(F)$, contradicting proper separation for $b$. For $\lambda=1$, the same argument uses proper separation for $a$; for $\lambda=0$, it uses proper separation for $b$. Hence $h_\lambda$ is non-constant on $\aff(F)$, and $F_\lambda$ has affine dimension below $d$.
...
Fix such a game. By \cref{lem:pbe-existence,prop:pbe-characterization}, it has an individually rational PBE partition with strictly decreasing payoffs. The proof of \cref{lem:thinB-value-set} depends only on Bayes plausibility and \cref{prop:thinB-common-value}, so it applies unchanged to this game and its residual games.
...
By \cref{lem:thinB-A4}, $G$ satisfies the standing regularity assumption, and by \cref{lem:thinB-envelope-B} its upper envelope $\overline v$ satisfies B. By the additional hypothesis of the theorem, $\overline v$ is continuous. The geometric construction in the proof of \cref{lem:btw-order} depends only on continuity and B of the function being ranked, so it applies to every level set of $\overline v$.

Construct a partition recursively. Let $R_1:=\Theta$. Given non-empty $R_t$, set
\[
    w_t:=\max\W_{R_t}.
\]
The maximum exists by \cref{lem:coalition-compact}. If $(C,\sigma,w_t)$ attains it, \cref{lem:thinB-value-set} allows the same strategy to be paid $\overline v(\mu^0_C)$. Maximality and $w_t\leq\overline v(\mu^0_C)$ imply
\[
...
\[
    ...
    \Bigr).
\]
It is a disclosure game. The relative-preimage identity says that every type in $D$ has an auxiliary message, and every auxiliary message is used by one of the two original coalitions and hence is available to a type in $D$.

By \cref{prop:thinB-upper-pbe}, $G'$ has a PBE partition
\[
    \{(\widetilde C_\tau,\widetilde\sigma_\tau,\widetilde w_\tau)\}_{\tau=1}^k
\]
such that
