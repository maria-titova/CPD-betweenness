...
\Cref{thm:two-B,thm:thinB-cppbe} give the single-valued and thin-B versions of the result. Under single-valued B, $V=\{v\}$ and continuity follows from \cref{A4:V-uhc}; under thin-B, the generator may be discontinuous and $V$ records its limiting values. The thin-B proof does not replace $V$ by the generally invalid singleton correspondence $\{\overline v\}$. Instead, \cref{prop:thinB-upper-pbe} normalizes genuine PBE partitions of the correspondence-valued game, while the upper-semicontinuous version of the level-set order in \cref{lem:btw-order} ranks the resulting upper-envelope values.
...
\begin{lemma}\label{lem:btw-order}\ \leanmixed{BetweennessOrder.lean::btw\_order}{BetweennessOrderUSC.lean::btw\_order\_usc}
...
By \cref{lem:thinB-A4}, $G$ satisfies the standing regularity assumption and its upper envelope $\overline v$ is upper semicontinuous. By \cref{lem:thinB-envelope-B}, $\overline v$ satisfies the betweenness inequalities. Hence \cref{lem:btw-order} applies to every level set of $\overline v$.
