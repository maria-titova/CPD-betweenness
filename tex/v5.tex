...
\begin{assumption}\label{assumption:game}
...
\begin{definition}\label{dfn:game}
...
\begin{definition}\label{dfn:coalition}
...
\begin{remark}\label{remark:coalitions-exist}
...
\begin{remark}\label{remark:partitions-exist}
...
\begin{definition}\label{dfn:set-feasible-beliefs}
...
\begin{definition}\label{dfn:skeptical-payoff}
...
\begin{definition}\label{dfn:PBE}
...
\begin{proposition}\label{prop:pbe-characterization}
...
\begin{lemma}\label{lem:strong-IR}
...
\begin{definition}\label{dfn:blocking}
...
\begin{proposition}\label{prop:cppbe-characterization}
...
\begin{definition}\label{dfn:generic}
    Game $G$ is \emph{generic} if the map $C\mapsto\overline{v}(\mu^0_C)$ is injective on non-empty subsets $C\subseteq\Theta$.
\end{definition}

\subsection{Betweenness}\label{sec:existence-betweenness}

We next strengthen the payoff restriction and, in return, impose no condition on the message mapping. The strengthened restriction is betweenness: the payoff of a mixture lies between the endpoint payoffs, or equivalently all upper and lower level sets of $v$ are convex. It applies to a single-valued payoff, so throughout this subsection $V=\{v\}$; the two envelopes coincide, $\overline v=\underline v=v$, and $v$ is continuous by \cref{A4:V-uhc}. We impose single-valuedness because the disclosure papers whose existence arguments use betweenness, \textcite{hart2017} and \textcite{bertomeucianciaruso}, both work with a real-valued payoff.\footnote{\textcite{hart2017} give a microfoundation: betweenness holds whenever the receiver's expected utility is single-peaked in her action at every belief, and for $|\Theta|=2$ it reduces to monotonicity of $v$ along the simplex. Their single-valued payoff is the receiver's single-peaked reward as a function of the belief; \textcite{bertomeucianciaruso} impose their convexity condition (CV), requiring the value of a merged belief to lie between the worst and best values of its components, on a real-valued payoff.}

\begin{definition}\label{dfn:B}
...
\begin{lemma}\label{lem:value-id}
...
\begin{lemma}\label{lem:btw-attained}
...
\begin{lemma}\label{lem:btw-merging}
...
\begin{lemma}\label{lem:remaining-games}
...
\begin{definition}\label{dfn:coe}
...
\begin{lemma}\label{lem:coe-greedy}
    \begin{enumerate}
...
\begin{lemma}\label{lem:btw-order}
...
\begin{lemma}\label{lem:feasible-polytope}
...
\begin{lemma}\label{lem:pbe-existence}
