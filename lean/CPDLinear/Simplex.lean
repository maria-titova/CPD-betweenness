...
/-!
# The standard simplex on a `Finset`  (writeup §1, "For a finite set `S`…")

For a finite set `S`, `Δ S` is the set of probability distributions on `S`,
encoded as functions `α → ℝ` that are nonnegative, sum to one, and vanish
outside `S` (the *zero-extension* encoding).  `simplexSupport μ = {a | μ a > 0}`.
-/
