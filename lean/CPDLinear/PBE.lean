...
/-!
# PBE strategies  (writeup §3: Perfect Bayesian Equilibrium)

`argmax_{m∈S} r(m)`; the predicate `Supports σ μ r` packaging the four PBE
conditions (feasible beliefs, Bayesian on path, payoff-compatible, sequentially
rational); and `IsPBE σ`, the existential form (Def PBE).
-/
...
/-- **`(μ, r)` supports `σ` as a PBE** (the four conditions of Def PBE). -/
...
variable (G) in
/-- **`σ` is a PBE strategy**: some `(μ, r)` supports it. -/
