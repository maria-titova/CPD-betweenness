...
/-!
# Ranking level sets under plain betweenness — axiom-free core (writeup §7)

This module isolates the *axiom-free* geometric machinery behind `lem:btw-order`:
point/convex-set separation (`proper_separation`), convexity of sub/super level
sets of `v̄`, the level-component extraction lemma, and the generalized ranking
construction `btw_order_aux`.  It imports only `CPDLinear.Betweenness` (which
does not depend on the `kakutani` axiom), so these results can be developed
independently of the fixed-point machinery.  `CPDLinear.BetweennessOrder`
consumes `btw_order_aux` to prove the public `btw_order`.
-/
...
/-- **`lem:proper-separation`.** Non-empty disjoint convex subsets of `ℝ^Θ`
admit a *proper* affine separation: `h ≤ 0` on `A₁`, `h ≥ 0` on `A₂`, and `h`
not identically zero on `A₁ ∪ A₂`. (Rockafellar, Thm. 11.3; the sets are
neither open nor compact in general.) -/
...
/-- The separator-ranking relation: rank primarily by the affine separator `f`
(descending), breaking ties on the fibre `{f = f a}` with the recursively supplied
slice order `σ (f a)`. -/
...
Round 6 established that the single-separator alignment lemma `sep_comonotone`
is **false** under plain betweenness (the Möbius/plateau wedge counterexample):
on a full-dimensional level "wedge" the unique proper separator is not
co-monotone with `v̄`, so any single-separator ranking is unsound.
...
/-- Pencil coordinate of `x` from the two separators `a = fp + cp`, `b = fm + cm`:
for a level point (`a x ≤ 0 ≤ b x`) it lies in `[0,1]`; the degenerate point
`a x = b x = 0` is sent to the top value `1`. -/
noncomputable def pencilLam (fp fm : (T → ℝ) →ₗ[ℝ] ℝ) (cp cm : ℝ) (x : T → ℝ) : ℝ :=
  if (fm x + cm) - (fp x + cp) = 0 then 1
  else (fm x + cm) / ((fm x + cm) - (fp x + cp))

/-- Affine pencil functional at parameter `t`:
`pencilVal t x = t·(a x) + (1-t)·(b x)` where `a = fp + cp`, `b = fm + cm`. -/
def pencilVal (fp fm : (T → ℝ) →ₗ[ℝ] ℝ) (cp cm : ℝ) (t : ℝ) (x : T → ℝ) : ℝ :=
  t * (fp x + cp) + (1 - t) * (fm x + cm)

/-- The pencil ranking relation: rank primarily by `pencilLam` (descending),
breaking ties on the pencil hyperplane `{pencilVal t = 0}` (with `t = pencilLam a`)
using the recursively supplied slice order `σ (pencilLam a)`. -/
