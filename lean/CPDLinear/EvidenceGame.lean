...
Formalizes the comparison with Hart–Kremer–Perry (2017): **Definition 25**
...
`EvidenceStructure`; **Definition 26** (truth-leaning equilibrium: a PBE
...
`hkp_dichotomy`; **Proposition 7** (every truth-leaning equilibrium strategy is
...
variable (G) in
/-- **Definition 25** (evidence game). The message mapping has *evidence
...
/-- **Definition 26** (truth-leaning equilibrium). A PBE `(σ, μ, r)` (the
...
/-- **Pooling-value bound (the core of Proposition 7).** In a truth-leaning
...
/-
**Proposition 7.** The sender's strategy in every truth-leaning equilibrium
