...
/-!
# Cheap talk and the quasiconcave closure  (writeup §6, subsection Cheap Talk)

* `MCT` — cheap-talk copies (dfn:mct): every `m ∈ 𝓜` has at least `n := |Θ|`
  messages with the same preimage.
* `qcClosure` (`v̄^qc`) — the quasiconcave closure of the upper envelope
  (dfn:qc-closure), and the closure game `G^qc` (lem:closure-game).
* `coalition_w_le_qcClosure` (lem:qc-bound) and `ct_realization`
  (lem:ct-realization).
-/
...
variable (G) in
/-- **(M-CT)** cheap-talk copies: for every `m ∈ 𝓜`, at least `n := |Θ|`
messages `m'` share the preimage `P(m')=P(m)`. -/
def MCT : Prop :=
...
variable (G) in
/-- The **quasiconcave closure** `v̄^qc(μ)` (dfn:qc-closure): the supremum of the
objective over the constraint set. -/
...
private lemma vbarUpperLevel_isCompact (c : ℝ) : IsCompact (G.vbarUpperLevel c) := by
  convert ( UpperSemicontinuousOn.isCompact_inter_preimage_Ici G.vbar_upperSemicontinuousOn ( CPDLinear.isCompact_simplexOn G.Θ ) c ) using 1
...
private lemma condPrior_mem_convexHull_balanced {C : Finset T} (hC : C.Nonempty)
  ...
  · refine' subset_convexHull ℝ _ _;
    ...
    · exact CPDLinear.simplexSupport_subset ( CPDLinear.DisclosureGame.condPrior_mem_simplex hC hCΘ );
  · obtain ⟨lp, hlp_constraint, hlp_qcObjective⟩ : ∃ lp ∈ G.qcConstraint (G.condPrior C), G.qcObjective lp = G.qcClosure (G.condPrior C) := by
      ...
      exact CPDLinear.DisclosureGame.condPrior_mem_simplex hC hCΘ |> CPDLinear.simplexOn_mono hCΘ;
    obtain ⟨t, ht_mem, ht_V⟩ : ∃ t : T → ℝ, (∀ θ₀ ∈ G.Θ, t θ₀ ∈ Set.Icc (0 : ℝ) 1) ∧ (∀ θ₀ ∈ G.Θ, G.qcClosure (G.condPrior C) ∈ G.V (fun θ => (1 - t θ₀) * lp.2 θ₀ θ + t θ₀ * G.condPrior C θ)) ∧ (∀ θ₀ ∈ G.Θ, t θ₀ ≠ 1) := by
      have ht_exists : ∀ θ₀ ∈ G.Θ, ∃ t ∈ Set.Icc (0 : ℝ) 1, G.qcClosure (G.condPrior C) ∈ G.V (fun θ => (1 - t) * lp.2 θ₀ θ + t * G.condPrior C θ) ∧ t ≠ 1 := by
        ...
        have hwμ : G.vlow (G.condPrior C) ≤ G.qcClosure (G.condPrior C) := by
          ...
          apply_rules [ CPDLinear.DisclosureGame.vlow_le, CPDLinear.DisclosureGame.vbar_mem ]; all_goals exact CPDLinear.DisclosureGame.condPrior_mem_simplex hC hCΘ |> CPDLinear.simplexOn_mono hCΘ;
...
/-! ## The closure game (lem:closure-game (ii)) -/

variable (G) in
/-- **The closure game** `G^qc := (Θ, 𝓜, M, μ⁰, V^qc)` with
`V^qc(μ) := [v_min, v̄^qc(μ)]`. -/
...
/-! ## lem:closure-game (i): closure properties -/

/-- **lem:closure-game (i).** `v̄^qc` is upper semicontinuous on `ΔΘ`. -/
lemma qcClosure_upperSemicontinuousOn :
    UpperSemicontinuousOn G.qcClosure (simplexOn G.Θ) :=
  qcClosure_usc_aux

/-
**lem:closure-game (i).** `v̄^qc` is quasiconcave.
-/
...
/-- **lem:closure-game (i).** Superlevel identity:
`{v̄^qc ≥ y} = conv{v̄ ≥ y}`. -/
...
/-
**lem:closure-game (ii).** The upper envelope of `G^qc` is `v̄^qc`.
-/
...
/-
**lem:closure-game (ii).** `G^qc` is quasiconcave.
-/
...
/-
**lem:closure-game (iii).** M-C of `G` gives M-C of `G^qc`.
-/
...
/-! ## lem:qc-bound and lem:ct-realization -/

/-- **lem:qc-bound.** For every non-empty `R ⊆ Θ` and every coalition `(C,σ,w)`
of `G|_R`, `w ≤ v̄^qc(μ⁰_C)` (the closure is of `G`'s upper envelope, `μ⁰_C` the
conditional prior of `μ⁰`). -/
...
/-
**lem:ct-realization.** Under M-CT, for non-empty `R ⊆ Θ` and `m̄ ∈ 𝓜_R`,
the game `G|_R` has a coalition with type set `P(m̄) ∩ R` and payoff
`v̄^qc(μ⁰_{P(m̄)∩R})`.
-/
