...
/-!
# Betweenness core (writeup §7, `sec:betweenness`)

The single-valued betweenness development of writeup §7: value identification
(`lem:value-id`), the largest pooling value over relative preimages `v*`
(`dfn:vstar`), attainment (`lem:btw-attained`), and the merging lemma
(`lem:btw-merging`, construction and B*-impossibility halves).

Betweenness (B) and strict betweenness (B*) themselves — `dfn:B`, `dfn:Bstar` —
are `Betweenness` / `StrictBetweenness` in `CPDLinear/Betweenness.lean`.

Statements scaffolded for call14 (see `calls/call14-betweenness/SPEC.md`);
proofs pending Aristotle.
-/
...
variable (G) in
/-- **`V` is single-valued** (the standing assumption of writeup §7): at every
belief in `ΔΘ`, `V(μ)` is the singleton `{v̄(μ)}` (so `v := v̄ = v̲`). -/
...
/-- **Key inequality (`lem:btw-attained` core).** Under single-valued `V` and
betweenness, the pooling value of every relative-preimage cell is bounded by the
greatest attainable coalition payoff of `G|_R`.

This is the single remaining gap of the §7 development.  It is the
message-completeness-free *realization* step: the pooling value `v̄(μ⁰_{M⁻¹_R(X)})`
of a relative-preimage cell is dominated by an attainable coalition payoff.  For
cells that split into components with no shared messages this follows from the
betweenness convex-combination bound and single-message pooling
(`btwc_single_pool_mem`, `btwc_condPrior_union`); but for a *connected* cell it
requires constructing a value-equalizing coalition strategy (a "bridge" type
splitting its messages so that all induced beliefs share a common `v̄`), whose
existence is a fixed-point-level statement.  The single-valued hypothesis makes
`v̄` continuous (`btwc_vbar_continuousOn`); the construction is the
...
/-! ## Value identification (`lem:value-id`) -/

/-
**`lem:value-id`.** Assume `V` is single-valued and `v̄` satisfies B. Then
every coalition `(C, σ, w)` of `G` satisfies `w = v(μ⁰_C)`.
-/
...
/-! ## The largest pooling value over relative preimages (`dfn:vstar`) -/

variable (G) in
/-- **`dfn:vstar`.** `v*(R) := max_{∅ ≠ X ⊆ 𝓜_R} v(μ⁰_{M⁻¹_R(X)})`, the largest
pooling value over relative preimages (encoded as an `sSup`; the max over the
finitely many non-empty `X ⊆ 𝓜_R` is attained, see `vstar_isGreatest`). -/
...
/-
The maximum in `dfn:vstar` is attained: the candidate set is a non-empty
finite set of reals (every `m ∈ 𝓜_R` is available to some type in `R`, so every
relative preimage is non-empty).
-/
...
/-! ## Attainment (`lem:btw-attained`) -/

/-
**`lem:btw-attained`, bound half.** Assume `V` is single-valued and `v̄`
satisfies B. Every coalition `(C, σ, w)` of `G|_R` satisfies `w = v(μ⁰_C)` and
`w ≤ v*(R)`.
-/
...
/-
**`lem:btw-attained`, attainment half.** Assume `V` is single-valued and
`v̄` satisfies B. Then `max 𝒲_R = v*(R)`.
-/
...
/-! ## Merging (`lem:btw-merging`) -/

/-
**`lem:btw-merging`, construction half.** Assume `V` is single-valued and
`v̄` satisfies B. Let `(C, σ, w)` be a coalition of `G|_R` with `w = max 𝒲_R`,
and suppose `R' := R ∖ C` is non-empty and `G|_{R'}` admits a coalition
`(C', σ', w')` with `w' > w`. Then `M⁻¹_R(X(σ) ∪ X(σ')) = C ∪ C'` and
`v(μ⁰_{C ∪ C'}) = w`.
-/
...
/-
**`lem:btw-merging`, impossibility half.** Under strict betweenness the
configuration of `btw_merging` is impossible: no residual coalition pays
strictly more than the maximal payoff.
-/
