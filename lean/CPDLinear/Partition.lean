...
/-!
# Partitions  (writeup §2: Partitions, Algorithm, partition strategy)

A **partition** `Π = {(C_t, σ_t, w_t)}_{t=1..T}`: pairwise-disjoint non-empty
cells covering `Θ`, each `(C_t,σ_t,w_t)` a coalition of the residual game
`G_t := G|_{R_t}` with `R_t := ⋃_{s≥t} C_s` (`thetaStep`).  The **associated
partition strategy** `σ^Π` (Def partition-strategy), the existence/step-bound
(Remark partitions-exist), and Lemma partition-strategy (distinct steps use
disjoint evidence; for `m ∈ X_t` only types in `C_t` send `m`, so `m` is on path
and `μ_{σ^Π}(·|m) = μ^{C_t,σ_t}(·|m)`).
-/
...
/-- A **partition** of `G`: `Π = {(C_t, σ_t, w_t)}_{t=1..T}`. -/
...
/-- The **associated partition strategy** `σ^Π : Θ → Δ𝓜`. -/
...
lemma evidence_disjoint {s t : Fin P.card} (hst : s ≠ t) :
  ...
  · obtain ⟨θ, hθ⟩ : ∃ θ ∈ P.C s, 0 < (P.σ s).σ θ m := by
    ...
    have hθ_in_preimage : θ ∈ G.preimageSet (thetaStep P.C t) (P.σ t).evidence := by
      ...
      exact ⟨ hθ_in_Rt, m, by
        have := (P.σ s).mem θ hθ.1; simp_all +decide [ CPDLinear.simplexOn ] ;
...
/-
**Remark (partitions exist).** Every disclosure game admits a partition.
-/
