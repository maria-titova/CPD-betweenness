...
/-!
# Degradability and Free Disposal  (writeup §6, "Degradability and Free Disposal")

Sufficient conditions for existence that constrain only the lower envelope `v̲`.
The common mechanism is **payoff degradation**: the ability to lower a coalition's
payoff while keeping its members.  We record the abstract degradation property,
show it makes every greedy run complete (`degrade_halts`), and give two sufficient
conditions for it: cheap talk + degradability (`degrade_general`) and free disposal
/ revelation aversion (the corollaries live in `ExistenceDegrading`).

This subsection is flagged *provisional* in the source draft.
-/
...
/-- **Degradability (dfn:degradable).** `V` is *degradable* if every belief `q`
with `v̄(q) ≥ w' ≥ v_min` is a convex combination of beliefs, each supported inside
`supp q` and paying `w'`. -/
def Degradable : Prop :=
...
/-- **Free disposal (dfn:FD).** The lower envelope is constant at the floor:
`v̲(μ) = v_min` for every `μ`. -/
def FreeDisposal : Prop := ∀ μ ∈ simplexOn G.Θ, G.vlow μ = G.vMin

/-- **Revelation aversion (dfn:revaverse).** `v̲(δ_θ) = v_min` for every `θ ∈ Θ`. -/
def RevelationAverse : Prop := ∀ θ ∈ G.Θ, G.vlow (pointMass θ) = G.vMin

/-- The abstract **payoff-degradation property**, the hypothesis of
`degrade_halts`: in every restricted game `G|_R`, every coalition `(C,σ,w)` can be
re-priced down to any `w' ∈ [v_min, w]` keeping its type set `C`. -/
...
/-
**lem:degrade-halts.** Under the payoff-degradation property, no run of the
greedy algorithm halts, so `G` admits a greedy partition and a coalition-proof PBE
exists.
-/
...
/-- **lem:degrade-general.** Cheap-talk copies plus degradability of `V` yield the
payoff-degradation property. -/
