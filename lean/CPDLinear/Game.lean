...
/-!
# Disclosure games  (writeup §1: Model, Assumptions, Envelopes)

A disclosure game `G = (Θ, 𝓜, M, μ⁰, V)`: finite non-empty type/message spaces,
a message mapping `M`, a full-support prior `μ⁰`, and a payoff correspondence
`V : ΔΘ ⇉ ℝ` that is upper hemicontinuous with non-empty compact interval values
(Assumption (A1)–(A4)).  The upper/lower envelopes are `v̄ = max V`, `v̲ = min V`.
The *envelope properties* taken as primitive in the writeup
(`V(μ) = [v̲(μ), v̄(μ)]`, `v̄` u.s.c., `v̲` l.s.c.) are recorded here as lemmas
to be derived from (A4).
-/
...
/-- A **disclosure game** `G = (Θ, 𝓜, M, μ⁰, V)` (Assumption (A1)–(A4)). -/
...
/-
`v̄(μ) = max V(μ)`: the upper envelope is attained.
-/
...
/-
`v̲(μ) = min V(μ)`: the lower envelope is attained.
-/
...
/-
**Envelope interval (Assumption envelopes).** `V(μ) = [v̲(μ), v̄(μ)]`.
-/
...
/-
**Envelope semicontinuity (upper).** `v̄` is upper semicontinuous on `Δ Θ`.
-/
...
/-
**Envelope semicontinuity (lower).** `v̲` is lower semicontinuous on `Δ Θ`.
-/
