...
/-!
# Auxiliary results for lexicographically maximal PBE partitions (working-paper appendix)

Lexicographically maximal PBE partitions are working-paper appendix material
(unnumbered): when no coalition-proof PBE exists, they are the natural
...
file collects the *axiom-free* machinery used by `CPDLinear.LexMax` to build one:
