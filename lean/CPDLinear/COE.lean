...
/-!
# Coalition-optimal equilibrium partitions  (writeup §5)

Lemma remaining-games (forced types stay; conditional prior is feasible; `𝒲_t`
non-empty/compact with `max 𝒲_t ≥ max u̲`), the COE partitions (Def coe), and
Lemma coe-greedy: every COE partition is greedy, and a greedy partition is COE
iff `max 𝒲_t ≤ w_{t-1}` for every `t`.
-/
...
/-- `max 𝒲_{R_t}`, the maximum of the remaining coalition-payoff set. -/
noncomputable def stepMax (t : Fin P.card) : ℝ := sSup (P.stepPayoffs t)

/-
**(iii)** `𝒲_{R_t}` is non-empty and compact, and
...
/-- **Definition (COE partition).** For every `t`, `w_t = max 𝒲_{R_t}` and
`w_t ≤ w_{t-1}`. -/
def IsCOE : Prop :=
...
/-
**Lemma coe-greedy (i).** Every COE partition is greedy.
-/
...
/-
**Lemma coe-greedy (ii).** A greedy partition is COE iff
