...
/-!
# Existence of Coalition-Proof PBE — Quasiconcavity and Message Completeness
  (writeup §6, first part: conditions QC/QC*/M-C, genericity, pooling, merging)

Definitions of the conditions QC, QC*, M-C and genericity, together with the
**pooling-dominance** lemma (`lem:pooling`) and the **merging** lemma
(`lem:merging`).  Here `P(m)` is `G.canSend m`, `μ⁰_C` is `G.condPrior C`, `v̄`
is `G.vbar`, and `𝒲` is `G.coalitionPayoffs`.
-/
...
variable (G) in
/-- **(QC)** `v̄` is *quasiconcave* on `ΔΘ`. -/
def QC : Prop :=
  ∀ μ ∈ simplexOn G.Θ, ∀ μ' ∈ simplexOn G.Θ, ∀ l : ℝ, l ∈ Set.Ioo (0 : ℝ) 1 →
    min (G.vbar μ) (G.vbar μ') ≤ G.vbar (fun θ => l * μ θ + (1 - l) * μ' θ)

variable (G) in
/-- **(QC*)** the inequality in (QC) is strict whenever `μ ≠ μ'`. -/
def QCStar : Prop :=
  ∀ μ ∈ simplexOn G.Θ, ∀ μ' ∈ simplexOn G.Θ, μ ≠ μ' → ∀ l : ℝ, l ∈ Set.Ioo (0 : ℝ) 1 →
    min (G.vbar μ) (G.vbar μ') < G.vbar (fun θ => l * μ θ + (1 - l) * μ' θ)

variable (G) in
/-- **(M-C)** the message structure is *complete*: for all `m, m' ∈ 𝓜` there is
`m'' ∈ 𝓜` with `P(m'') = P(m) ∪ P(m')`. -/
def MC : Prop :=
  ∀ m ∈ G.𝓜, ∀ m' ∈ G.𝓜, ∃ m'' ∈ G.𝓜, G.canSend m'' = G.canSend m ∪ G.canSend m'

variable (G) in
/-- **Genericity**: `C ↦ v̄(μ⁰_C)` is injective on non-empty subsets `C ⊆ Θ`. -/
def Generic : Prop :=
...
/-! ## Lemma (pooling): pooling dominance and its consequences -/
...
/-
**Pooling dominance (lem:pooling, main part).** Under QC and M-C, every
coalition `(C, σ, w)` admits a message `m*` with `P(m*) = C`, and `w` is
dominated by `v̄(μ⁰_C)`.
-/
...
/-
**lem:pooling, identity.** Under QC and M-C, the greatest coalition payoff is
the greatest pooling payoff: `max 𝒲_Θ = max_{m∈𝓜} v̄(μ⁰_{P(m)})`.
-/
...
/-
**lem:pooling, last claim.** Under QC and M-C, every coalition attaining
`max 𝒲_Θ` has `v̄(μ⁰_C) = max 𝒲_Θ`.
-/
...
/-
**Merging (lem:merging).** Under QC and M-C, from a coalition of `G|_R`
attaining `max 𝒲_R` and a strictly higher-paying coalition on `R ∖ C`, one
builds a strictly larger coalition of `G|_R` with the same payoff and equal
pooling value; under QC* or genericity this is impossible (so removing a cell
attaining `max 𝒲_R` cannot raise the residual maximum).
-/
