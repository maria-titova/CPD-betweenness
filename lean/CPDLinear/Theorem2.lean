...
/-!
# Theorem 2: existence under strict betweenness (writeup `thm:two`,
`thm:betweenness-coe`)

Under single-valued `V` and strict betweenness (no message-completeness
assumption): no run of the greedy algorithm halts, every greedy step attains
the unconstrained residual maximum, a coalition-proof PBE exists, the
coalition-proof partition is essentially unique under genericity, and a COE
partition exists.

Statements scaffolded for call16 (see `calls/call16-theorem2/SPEC.md`);
...
/-- **`thm:two`, extension step.** Under single-valued `V` and B*, every
...
/-- **`thm:two`, full no-halt form.** Under single-valued `V` and B*, every
...
/-- **`thm:two`, step characterization.** Under single-valued `V` and B*,
...
/-! ### Remaining `thm:two` / `cor:bstar-coe` statements -/

/-- **`thm:two`, existence.** Under single-valued `V` and B*, a
...
/-- **`thm:two`, uniqueness.** Under single-valued `V`, B*, and genericity,
the coalition-proof PBE partition is essentially unique (mirrors
`one_unique`). -/
...
/-- **`cor:bstar-coe`.** Under single-valued `V` and B*, the COE notion is a
characterization: a partition is a coalition-proof PBE partition iff it is a
COE partition. -/
...
/-- **`thm:betweenness-coe`.** Under single-valued `V` and B*, `G` admits a
COE partition. -/
