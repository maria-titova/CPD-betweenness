...
/-!
# Existence of PBE  (writeup §3, Lemma pbe-existence)

Every disclosure game admits a PBE.  This is the unique place where Kakutani's
fixed-point theorem (`kakutani`) is used: one applies it to the best-response
correspondence `Φ` on `K = Σ × ∏_m 𝓕(m) × ∏_m I` and reads a PBE off a fixed
point.
-/
...
lemma exPBE_readoff {p : FixedPointSpace T Msg} (hpK : p ∈ exPBE_K G)
  ...
  · intro m hm
    have hm_mem : m ∈ G.𝓜 := by
      ...
      exact G.M_subset θ hθ ( CPDLinear.simplexSupport_subset ( hpK.1 θ hθ ) hm )
...
/-- **Lemma pbe-existence.** Every disclosure game admits a PBE. -/
