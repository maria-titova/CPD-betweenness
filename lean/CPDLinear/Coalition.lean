...
/-!
# Coalitions  (writeup §2: Coalitions and Partitions)

A **coalition strategy** on a non-empty `C ⊆ Θ` is a sender strategy of `G|_C`;
its **coalition-induced belief** `μ^{C,σ}(·|m) := ι_C(μ_σ(·|m))` is the
zero-extension to `Θ`.  A **coalition** `(C, σ, w)` of `G` (Def coalition,
conditions (C1)–(C4)).  The reverse inclusion `C ⊆ M⁻¹(X(σ))` is automatic, so
`M⁻¹(X(σ)) = C`.  Every disclosure game admits a coalition (Remark
coalitions-exist).
-/
...
lemma mem_preimageSet {S : Finset T} {X : Set Msg} {θ : T} :
  ...
  unfold CPDLinear.DisclosureGame.preimageSet; simp +decide [ Set.ext_iff ] ;
...
variable (G) in
/-- A **coalition** `(C, σ, w)` of `G`: (C1) `C` non-empty, (C2) `σ` a coalition
strategy on `C`, (C3) evidence exclusive to `C`, (C4) common payoff `w`. -/
...
/-
**Remark (coalitions exist).** Every disclosure game admits a coalition.
-/
