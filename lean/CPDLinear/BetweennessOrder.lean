...
/-!
# Ranking level sets under plain betweenness (writeup §7, tie-breaking)

Under plain betweenness the merged pool of `lem:btw-merging` can sit exactly on
the level set `{v = max 𝒲_R}`, and existence depends on *which* payoff-maximal
coalition is selected. This module provides the tie-breaking selection:
proper separation (`lem:proper-separation`), the ranking of a level set
(`lem:btw-order` — an existence statement; the construction inducts on affine
dimension), and existence of a coalition-proof PBE under single-valued `V` and
plain B (`thm:two-B`).
-/
...
variable (G) in
/-- The level set `L = {μ ∈ ΔΘ | v(μ) = y}`. -/
def levelSet (y : ℝ) : Set (T → ℝ) :=
  {μ ∈ simplexOn G.Θ | G.vbar μ = y}

/-- **`lem:btw-order`** (Ranking a level set). Under single-valued `V` and B,
every level set `L = {v = y}` carries a complete transitive relation `⪰` such
that (i) moving from `x ∈ L` strictly toward the upper level set, while
...
`private` `btwcAux` development of `CPDLinear.BetweennessCore` (cloned here
...
`G'.V := G.V` (clone of the private `btwcAux` of `BetweennessCore`). -/
...
Proof (the writeup's tie-breaking): let `y = max 𝒲`.  `btw_order` provides a
...
/-- **`thm:two-B`.** Assume `V` is single-valued. If `v̄` satisfies plain
betweenness, then a coalition-proof PBE exists. -/
theorem two_B_existence (hSV : G.SingleValued) (hB : G.Betweenness) :
