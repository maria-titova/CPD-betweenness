...
Formalizes two results from the working-paper appendix that delimit the
existence theory. First, a minimal three-type non-existence example: a
...
/-- **The non-existence example** (working-paper appendix), base game. There
...
/-- **The non-existence example** (working-paper appendix), cheap-talk
...
private lemma one_type_coe (H : DisclosureGame T Msg) (hcard : H.Θ.card = 1) :
  ...
  exact t3_single_cell_coe H K hCΘ (hKw ▸ hw)
...
/-- **The two-type existence theorem** (working-paper appendix). If
...
theorem binary_existence (h2 : G.Θ.card = 2) :
  ...
  · obtain ⟨P, hP⟩ := t3_single_cell_coe G K hempty hKmax
  ...
  · have hne : (G.Θ \ K.C).Nonempty := Finset.nonempty_of_ne_empty hempty
    ...
    obtain ⟨P, hP⟩ := t3_prepend_coe G K hKmax hne hsub (hbound hne hsub) Pres hPres
    exact ⟨P, hP.isCPPBEPartition⟩
