...
/-!
# Sender strategies, evidence, and induced beliefs  (writeup §1: Strategies)

A sender strategy `σ : Θ → Δ𝓜` with `supp σ(·|θ) ⊆ M(θ)`; its evidence
`X(σ) = ⋃_θ supp σ(·|θ)`, the on-path probability `p_σ(m)`, and the induced
belief `μ_σ(·|m)`.
-/
...
/-- A **sender strategy** `σ : Θ → Δ𝓜` with `supp σ(·|θ) ⊆ M(θ)`. -/
...
/-- The **evidence** of `σ`: `X(σ) := ⋃_{θ ∈ Θ} supp σ(·|θ)`. -/
def evidence : Set Msg := ⋃ θ ∈ G.Θ, s.msgSupport θ

/-- The on-path probability `p_σ(m) := ∑_{θ ∈ Θ} μ⁰(θ) σ(m|θ)`. -/
noncomputable def onPathProb (m : Msg) : ℝ := ∑ θ ∈ G.Θ, G.μ0 θ * s.σ θ m

/-
**On-path messages are exactly the evidence.**
-/
...
/-- The induced (Bayesian) belief `μ_σ(θ|m) := μ⁰(θ) σ(m|θ) / p_σ(m)`. -/
noncomputable def belief (m : Msg) : T → ℝ :=
  fun θ => G.μ0 θ * s.σ θ m / s.onPathProb m

/-
**The induced belief is a probability distribution** in `Δ Θ`.
-/
