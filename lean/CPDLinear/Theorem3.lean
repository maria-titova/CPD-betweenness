...
/-!
# Theorem 3 and the pure-cheap-talk corollary  (writeup §6)

* `thm:three` — under M-C and M-CT a coalition-proof PBE exists.
* `cor:pure-ct` — if every message is available to every type and `|𝓜| ≥ n`, a
  coalition-proof PBE exists and is essentially unique (single cell `Θ` with
  payoff `v̄^qc(μ⁰)`).
-/
...
lemma t3_strategy_of_eq {g₁ g₂ : DisclosureGame T Msg} (h : g₁ = g₂) (s : Strategy g₁) :
    ∃ s' : Strategy g₂, s'.evidence = s.evidence ∧
      ∀ m, s'.coalitionBelief m = s.coalitionBelief m := by
  subst h; exact ⟨s, rfl, fun _ => rfl⟩

private lemma t3_coalition_of_eq {g₁ g₂ : DisclosureGame T Msg} (h : g₁ = g₂) (K : g₁.Coalition) :
    ∃ K' : g₂.Coalition, K'.C = K.C ∧ K'.w = K.w := by
  subst h; exact ⟨K, rfl, rfl⟩

lemma t3_coalitionPayoffs_of_eq {g₁ g₂ : DisclosureGame T Msg} (h : g₁ = g₂) :
    g₁.coalitionPayoffs = g₂.coalitionPayoffs := by
  subst h; rfl

/-! ### Restriction inheritance -/

private lemma t3_restrict_MC {S : Finset T} (hne : S.Nonempty) (hsub : S ⊆ G.Θ)
...
private lemma t3_restrict_QC {S : Finset T} (hne : S.Nonempty) (hsub : S ⊆ G.Θ)
    (hQC : G.QC) : (G.restrict S hne hsub).QC := by
  intro μ hμ μ' hμ' l hl
  exact hQC μ (simplexOn_mono hsub hμ) μ' (simplexOn_mono hsub hμ') l hl

private lemma t3_restrict_MCT {S : Finset T} (hne : S.Nonempty) (hsub : S ⊆ G.Θ)
...
-/
private lemma t3_coalitionPayoffs_subset {S : Finset T} (hne : S.Nonempty) (hsub : S ⊆ G.Θ) :
  ...
  exact ⟨ t3_vMin_le_vlow hν_mem |> le_trans <| hK_w.1, t3_vbar_le_qcClosure hν_mem |> le_trans hK_w.2 ⟩

/-- The upper envelope of the restricted closure game is `v̄^qc`. -/
private lemma t3_closure_restrict_vbar {S : Finset T} (hne : S.Nonempty) (hsub : S ⊆ G.Θ)
...
-/
private lemma t3_max_realizable {S : Finset T} (hne : S.Nonempty) (hsub : S ⊆ G.Θ)
  ...
  obtain ⟨mbar, hmbar⟩ : ∃ mbar ∈ (G.restrict S hne hsub).𝓜, K.w = (G.closureGame.restrict S hne hsub).vbar ((G.closureGame.restrict S hne hsub).condPrior ((G.closureGame.restrict S hne hsub).canSend mbar)) := by
    have h_pooling : IsGreatest (G.closureGame.restrict S hne hsub).poolingPayoffs K.w := by
      have h_pooling : (G.closureGame.restrict S hne hsub).QC := by
        grind +suggestions;
      convert isGreatest_coalitionPayoffs_iff h_pooling ( t3_restrict_MC hne hsub ( closureGame_MC hMC ) ) |>.1 hw;
  ...
  have h_vbar : (G.closureGame.restrict S hne hsub).vbar (G.condPrior (G.canSend mbar ∩ S)) = G.qcClosure (G.condPrior (G.canSend mbar ∩ S)) := by
    apply t3_closure_restrict_vbar;
...
-/
private lemma t3_isGreatest_closure_iff {S : Finset T} (hne : S.Nonempty) (hsub : S ⊆ G.Θ)
    (hMC : G.MC) (hMCT : G.MCT) {w : ℝ} :
  ...
  · obtain ⟨hw₁, hw₂⟩ := hw;
    refine' ⟨ _, fun x hx => _ ⟩;
    · exact t3_coalitionPayoffs_subset hne hsub hw₁;
    · obtain ⟨wB, hwB⟩ : ∃ wB, IsGreatest (G.closureGame.restrict S hne hsub).coalitionPayoffs wB := by
        ...
        exact ⟨ _, hx ⟩;
      have := t3_max_realizable hne hsub hMC hMCT hwB;
      exact le_trans ( hwB.2 hx ) ( hw₂ this );
  · refine' ⟨ t3_max_realizable hne hsub hMC hMCT hw, fun x hx => _ ⟩;
    exact hw.2 ( t3_coalitionPayoffs_subset hne hsub hx )

/-! ### Maximal cell -/

private lemma t3_exists_isGreatest_cp (H : DisclosureGame T Msg) :
...
-/
private lemma t3_qc_max_cell (H : DisclosureGame T Msg) (hQC : H.QC) (hMC : H.MC) :
...
private lemma t3_closure_attains (H : DisclosureGame T Msg) (hMC : H.MC)
...
-/
private lemma t3_g_max_cell (H : DisclosureGame T Msg) (hMC : H.MC) (hMCT : H.MCT) :
    ∃ K : H.Coalition, IsGreatest H.coalitionPayoffs K.w ∧
      ∀ (hne : (H.Θ \ K.C).Nonempty) (hsub : (H.Θ \ K.C) ⊆ H.Θ) (w' : ℝ),
        IsGreatest (H.restrict (H.Θ \ K.C) hne hsub).coalitionPayoffs w' → w' ≤ K.w := by
  obtain ⟨Kq, hKqmax, hqbound⟩ := t3_qc_max_cell H.closureGame (closureGame_QC) (closureGame_MC hMC);
  obtain ⟨m, hm, hcs⟩ := (pooling_dominance (DisclosureGame.closureGame_QC) (DisclosureGame.closureGame_MC hMC) Kq).1;
  obtain ⟨ K0, hK0C, hK0w ⟩ := CPDLinear.DisclosureGame.ct_realization hMCT H.Θ_nonempty ( subset_rfl ) ( show m ∈ H.restrictMsgSpace H.Θ from by
                                                                                                            simp +decide [ DisclosureGame.restrictMsgSpace ];
                                                                                                            have := H.cover; simp_all +decide [ Set.ext_iff ] ; );
  obtain ⟨K, hKC, hKw⟩ := t3_coalition_of_eq (restrict_self : H.restrict H.Θ H.Θ_nonempty subset_rfl = H) K0; use K; simp_all +decide [ Finset.inter_eq_left.mpr ( show H.canSend m ⊆ H.Θ from Finset.filter_subset _ _ ) ] ;
  refine' ⟨ _, _ ⟩;
  · convert t3_isGreatest_closure_iff H.Θ_nonempty ( subset_rfl ) hMC hMCT |>.mpr _ using 1;
    · exact t3_coalitionPayoffs_of_eq ( by simp +decide [ restrict_self ] );
    · convert t3_coalitionPayoffs_of_eq ( restrict_self : H.closureGame.restrict H.Θ H.Θ_nonempty subset_rfl = H.closureGame ) ▸ hKqmax using 1;
      rw [ t3_closure_attains H hMC Kq hKqmax, hcs.symm ];
  ...
  · intro hne w' hw'
    have hKqw : Kq.w = H.qcClosure (H.condPrior Kq.C) := by
      exact t3_closure_attains H hMC Kq hKqmax
    ...
    have hKqw''' : w' ≤ K.w := by
      ...
      · exact hKqw''.symm;
      · convert t3_isGreatest_closure_iff ( show ( H.Θ \ Kq.C ).Nonempty from hne ) ( show ( H.Θ \ Kq.C ) ⊆ H.Θ from Finset.sdiff_subset ) ( show H.MC from hMC ) ( show H.MCT from hMCT ) |>.1 hw' using 1
    exact hKqw'''.trans (by
    exact hKqw'.le)

/-! ### Generic COE construction (fresh private copies, no QC needed) -/

lemma t3_single_cell_coe (H : DisclosureGame T Msg) (K : H.Coalition)
  ...
  convert t3_coalitionPayoffs_of_eq _;
...
lemma t3_exists_prependP (H : DisclosureGame T Msg) (K : H.Coalition)
  ...
  rotate_left;
  exact fun t => Fin.cases K.σ ( fun j => ( t3_strategy_of_eq ( restrict_restrict hne hsub ( P'.C_nonempty j ) ( P'.C_subset j ) ) ( P'.σ j ) ).choose ) t;
...
lemma t3_prepend_isCOE (H : DisclosureGame T Msg) (K : H.Coalition)
  ...
  · simp_all +decide [ Partition.stepPayoffs ];
    convert hmax using 1;
    exact t3_coalitionPayoffs_of_eq ( restrict_self );
...
lemma t3_prepend_coe (H : DisclosureGame T Msg) (K : H.Coalition)
    (hmax : IsGreatest H.coalitionPayoffs K.w)
    (hne : (H.Θ \ K.C).Nonempty) (hsub : (H.Θ \ K.C) ⊆ H.Θ)
    (hbound : ∀ w', IsGreatest (H.restrict (H.Θ \ K.C) hne hsub).coalitionPayoffs w' → w' ≤ K.w)
    (P' : Partition (H.restrict (H.Θ \ K.C) hne hsub)) (hP' : P'.IsCOE) :
    ∃ P : Partition H, P.IsCOE := by
  obtain ⟨P, hcard, h0, hsucc⟩ := t3_exists_prependP H K hne hsub P'
  exact ⟨P, t3_prepend_isCOE H K hmax hne hsub hbound P' hP' P hcard h0 hsucc⟩

/-- **Existence of a COE partition under M-C and M-CT.** -/
private lemma t3_exists_coe (H : DisclosureGame T Msg) (hMC : H.MC) (hMCT : H.MCT) :
    ∃ P : Partition H, P.IsCOE := by
  induction' n : H.Θ.card using Nat.strong_induction_on with k ih generalizing H;
  obtain ⟨ K, hKmax, hbound ⟩ := t3_g_max_cell H hMC hMCT;
  ...
  · obtain ⟨hne, hsub⟩ : (H.Θ \ K.C).Nonempty ∧ (H.Θ \ K.C) ⊆ H.Θ :=
    ...
    exact ih _ hcard _ ( t3_restrict_MC hne hsub hMC ) ( t3_restrict_MCT hne hsub hMCT ) rfl
      |> fun ⟨ P, hP ⟩ => t3_prepend_coe H K hKmax hne hsub ( hbound hne hsub ) P hP

/-- **Theorem 3.** If the message mapping is complete (M-C) and has cheap-talk
copies (M-CT), then a coalition-proof PBE exists. -/
theorem three_existence (hMC : G.MC) (hMCT : G.MCT) :
    ∃ P : Partition G, P.IsCPPBEPartition := by
  obtain ⟨P, hP⟩ := t3_exists_coe G hMC hMCT
  exact ⟨P, hP.isCPPBEPartition⟩

/-! ### The pure-cheap-talk corollary -/

private lemma t3_pure_MC (hpure : ∀ m ∈ G.𝓜, G.canSend m = G.Θ) : G.MC := by
  intro m hm m' hm'
  use m;
  grind

private lemma t3_pure_MCT (hpure : ∀ m ∈ G.𝓜, G.canSend m = G.Θ)
...
-/
private lemma t3_pure_M_eq (hpure : ∀ m ∈ G.𝓜, G.canSend m = G.Θ) {θ : T} (hθ : θ ∈ G.Θ) :
    G.M θ = G.𝓜 := by
...
-/
private lemma t3_pure_coalition_C (hpure : ∀ m ∈ G.𝓜, G.canSend m = G.Θ)
...
-/
private lemma t3_pure_isGreatest (hpure : ∀ m ∈ G.𝓜, G.canSend m = G.Θ)
  ...
  · obtain ⟨m₀, hm₀⟩ : ∃ m₀ ∈ G.𝓜, True := by
    ...
    obtain ⟨K0, hK0⟩ : ∃ K0 : (G.restrict G.Θ G.Θ_nonempty (subset_refl G.Θ)).Coalition, K0.w = G.qcClosure G.μ0 := by
      have := ct_realization ( t3_pure_MCT hpure hcard ) G.Θ_nonempty ( subset_refl G.Θ ) ( show m₀ ∈ G.restrictMsgSpace G.Θ from ?_ );
      · grind +suggestions;
    ...
    obtain ⟨K, hK⟩ : ∃ K : Coalition G, K.C = K0.C ∧ K.w = K0.w := by
      convert t3_coalition_of_eq ( restrict_self : G.restrict G.Θ G.Θ_nonempty ( subset_refl G.Θ ) = G ) K0 using 1;
    exact ⟨ K, by aesop ⟩;
  · intro x hx;
    obtain ⟨K', hK'⟩ := hx;
    -- By `t3_pure_coalition_C hpure K'`, `K'.C = G.Θ`.
    have hK'_C : K'.C = G.Θ := by
      grind +suggestions;
    -- Transport `K'` along `restrict_self.symm` via `t3_coalition_of_eq` to `K'' : Coalition (G.restrict G.Θ G.Θ_nonempty subset_rfl)` with `K''.C = K'.C = G.Θ` and `K''.w = K'.w`.
...
-/
private lemma t3_pure_cell_eq_step (hpure : ∀ m ∈ G.𝓜, G.canSend m = G.Θ)
...
private lemma t3_pure_card_one (hpure : ∀ m ∈ G.𝓜, G.canSend m = G.Θ)
  ...
  -- By `t3_pure_cell_eq_step hpure P i`, `P.C i = thetaStep P.C i = (Finset.univ.filter (fun s => i ≤ s)).biUnion P.C`. Since `i = 0 ≤ j`, `P.C j ⊆ P.C i` (via `Finset.subset_biUnion_of_mem`).
  have h_subset : P.C (Fin.mk 1 (by omega)) ⊆ P.C (Fin.mk 0 (by omega)) := by
    ...
    convert t3_pure_cell_eq_step hpure P ⟨ 0, by linarith ⟩ using 1;
...
/-
**Corollary (pure cheap talk).** If `P(m) = Θ` for every `m` and
`|𝓜| ≥ n := |Θ|`, then a coalition-proof PBE exists and is essentially unique:
every coalition-proof PBE partition is the single cell `Θ` with payoff
`v̄^qc(μ⁰)`.
-/
theorem pure_ct (hpure : ∀ m ∈ G.𝓜, G.canSend m = G.Θ) (hcard : G.Θ.card ≤ G.𝓜.card) :
  ...
  refine' ⟨ _, fun P hP => _ ⟩;
  · exact ⟨_, (three_existence (t3_pure_MC hpure) (t3_pure_MCT hpure hcard)).choose_spec⟩
  · -- By definition of `IsCPPBEPartition`, we know that `P` is greedy.
    ...
    have hP_card : P.card = 1 := by
      apply t3_pure_card_one hpure P;
    refine' ⟨ hP_card, _, _ ⟩;
    · intro t;
      have hP_cover : G.Θ = Finset.univ.biUnion P.C := by
        exact P.cover_eq;
      convert t3_pure_cell_eq_step hpure P t;
    ...
    · intro t
      ...
      have hP_greedyLower : P.greedyLower t ≤ G.qcClosure G.μ0 := by
        have hP_stepMax : P.stepMax t = G.qcClosure G.μ0 := by
          have := t3_pure_isGreatest hpure hcard;
      ...
      have hP_greedyMax : IsGreatest (P.stepPayoffs t) (G.qcClosure G.μ0) := by
        convert t3_pure_isGreatest hpure hcard using 1
