...
/-!
# Attainable coalition payoffs  (writeup §4)

`𝒲_R := {w | (C,σ,w) is a coalition of G|_R for some C ⊆ R}`, here recorded as
the coalition-payoff set `coalitionPayoffs` of a game (apply to `G|_R` for the
subscript-`R` version).  Lemma coalition-compact: `𝒲_R` is non-empty and
compact.
-/
...
/-- The **set of attainable coalition payoffs** of `G`:
`𝒲 := {w | (C,σ,w) is a coalition of G}`. -/
...
/-- **Lemma coalition-compact: `𝒲_R` is compact.** -/
