...
/-!
# Tent path-independence (Lemma 9, Proposition 5)
...
* `faceMax_peak_consistency` (**Lemma 9**, peak consistency): if
  `∅ ≠ D ⊆ C ⊆ Θ` and `supp μ*_C ⊆ D`, then `μ*_D = μ*_C`.
* `tent_path_independence` (**Proposition 5**, tent path independence): any
...
/-- **Lemma 9** (peak consistency). Let `∅ ≠ D ⊆ C ⊆ Θ`. If `supp μ*_C ⊆ D`,
...
The proof of Proposition 5 (tent path independence) proceeds by strong induction on the size of the
...
private lemma FamRep.supp_subset {F : Γ.FaceMax} {ν : T → ℝ} (f : Γ.FamRep F ν) :
  ...
  have h_support_k : simplexSupport (f.b k) ⊆ (Γ.Θ : Set T) := by
    ...
    exact h_support_k.symm ▸ F.mem _ _ _ |>.1 |> fun h => CPDLinear.simplexSupport_subset h
...
/-- **Proposition 5** (tent path independence). Any two simplex
