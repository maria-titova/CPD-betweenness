...
/-!
# Preimages, forced types, conditional priors, restricted games  (writeup §1–§2)
...
* the **restricted message space** `𝓜_S`, the **zero-extension** `ι_S`, and the
  **restricted game** `G|_S` (Lemma: restriction preserves the class of games,
  `G|_Θ = G`, and restriction is transitive).
-/
...
lemma condPrior_of_not_mem {C : Finset T} {θ : T} (hθ : θ ∉ C) :
  ...
  unfold CPDLinear.DisclosureGame.condPrior;
  simp +decide [ hθ ]
...
lemma condPrior_mem_simplex {C : Finset T} (hC : C.Nonempty) (hCΘ : C ⊆ G.Θ) :
  ...
  have h_nonneg : ∀ θ, 0 ≤ G.condPrior C θ := by
    intro θ; unfold CPDLinear.DisclosureGame.condPrior; split_ifs <;> simp +decide [ *, div_nonneg, Finset.sum_nonneg, G.μ0_mem.1 ] ;
...
/-- The **restricted game** `G|_S` on a non-empty `S ⊆ Θ`. -/
...
/-
**Restriction is transitive**: `(G|_S)|_S' = G|_S'`.
-/
