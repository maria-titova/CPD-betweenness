...
/-!
# Coalition-proof PBE  (writeup §4)

`Θ(w̃)`, blocking coalitions (Def blocking), coalition-proofness, Lemma
cp-on-path, the greedy partitions (Def greedy), and Prop cppbe-characterization:
a partition is a coalition-proof PBE partition iff it is greedy.
-/
...
/-- **Definition (blocking).** A blocking coalition `(C̃, σ̃, w̃)` of `Π`:
`Θ(w̃) ≠ ∅` and `(C̃, σ̃, w̃)` is a coalition of `G|_{Θ(w̃)}` with payoff `w̃`. -/
...
/-- **Definition (coalition-proofness).** `Π` admits no blocking coalition. -/
...
/--
**Lemma cp-on-path.** A blocking coalition contains every on-path user, in
...
/-- **Definition (greedy partition).** `w_t = max(𝒲_{R_t} ∩ [max u̲, w_{t-1}])`. -/
...
/-- **Proposition cppbe-characterization.** A partition is a coalition-proof PBE
partition iff it is greedy. -/
...
/-- The strategy-level form of cppbe-characterization. -/
