...
/-!
# Greedy prefixes and the "no run halts" upgrades

The writeup preamble instructs: formalize a *greedy prefix* — a finite sequence
of coalitions of successive residual games whose payoffs satisfy the greedy
equation at every step, with a non-empty final residual — and prove, under the
relevant hypothesis, that the greedy constraint set at the final residual is
non-empty. This upgrades the existence-only conclusions `one_noHalt`
(Theorem1.lean) and `degrade_halts` (Degrading.lean) to the paper's "no run of
the greedy algorithm halts" clauses in thm:one(i) and thm:DEG-exists.

Statements scaffolded for call15 (see `calls/call15-greedy-prefix/SPEC.md`);
...
variable (G) in
/-- A **greedy-algorithm prefix**: finitely many pairwise-disjoint non-empty
cells `C_0, …, C_{card-1} ⊆ Θ` with a *non-empty final residual*
...
/-- **thm:one(i), extension step.** Under M-C and QC*, every greedy prefix has
a non-empty greedy constraint set at its final residual: no run of the greedy
algorithm halts. -/
...
/-- **thm:one(i), full form.** Under M-C and QC*, every greedy prefix extends
to a greedy partition of `G` (which is then a coalition-proof PBE partition by
`cppbe_characterization`): every run of the greedy algorithm terminates in a
...
/-! ## thm:DEG-exists, full "no run halts" form -/

/-
**thm:DEG-exists, extension step.** Under the payoff-degradation property,
every greedy prefix has a non-empty greedy constraint set at its final
residual (the writeup's displayed proof, already in prefix-extension form).
-/
...
/-- **thm:DEG-exists, full form.** Under the payoff-degradation property, every
greedy prefix extends to a greedy partition of `G`: no run of the greedy
algorithm halts. -/
