...
A standalone lightweight framework: rich disclosure games do **not** use the
`DisclosureGame` message machinery of the rest of the library. A rich
disclosure game fixes `(Θ, μ⁰, v̄)` — **Definition 21** — with coalitions
...
22**) greedily exhausts the prior. Under unique face maximizers and the tent
assumption, the induced tent function (**Definition 24**) is characterized on
`ΔΘ`.

Results formalized: **Lemma 8** (attainability: any belief on the current face
is induced by a sub-measure that retires a type) → `attainability`;
**Proposition 4** (existence of a coalition-optimal partition, reached
greedily in at most `|Θ|` steps, and uniqueness under unique face maximizers)
→ `cutecase_existence`, `cutecase_unique`; **Definition 23** (nested chain,
simplex decomposition) → `NestedChain`, `SimplexDecomp`,
`simplexDecomp_exists`; **Definition 24** and **Proposition 6** (the tent
...
/-- **Definition 21** (rich disclosure game). A *rich disclosure game*: a
...
/-- **Definition 21** (sub-measure). `η` is a *sub-measure* of `ν`:
`0 ≤ η ≤ ν` pointwise. -/
...
/-- **Definition 21** (coalition). A *coalition* of the restricted rich game
...
/-! ## Attainability (Lemma 8) -/

/-- **Lemma 8** (attainability). For nonzero `ν ≥ 0` and `μ ∈ Δ_ν` there is a
...
/-! ## Greedy machinery (private helpers for Proposition 4) -/
...
/-! ## Coalition-optimal partitions (Definition 22, Proposition 4) -/

variable (Γ) in
/-- **Definition 22** (coalition-optimal partition). A *coalition-optimal
...
variable (Γ) in
/-- **Proposition 4 (i)–(ii)**. Every rich disclosure game admits a
...
/-- **Proposition 4 (iii)**. Under unique maximizers the coalition-optimal
...
/-! ## Nested chains and simplex decompositions (Definition 23) -/
...
variable (Γ) in
/-- **Definition 23 (i)** (nested chain). Subsets
...
variable (Γ) in
/-- **Definition 23 (ii)** (simplex decomposition). A *simplex decomposition*
...
/-- **Definition 23** (existence). Every belief in `ΔΘ` admits a simplex
decomposition (greedy construction underlying Proposition 4, padding the
...
/-! ## The tent (Definition 24, Proposition 6) -/
...
condition needed for **Definition 24** (the tent) to be well defined. -/
def TentAssumption (F : Γ.FaceMax) : Prop :=
  ∀ (μ : T → ℝ) (d d' : Γ.SimplexDecomp F μ), d.value = d'.value

variable (Γ) in
/-- **Definition 24** (the tent). The *tent* of `v̄`: the common value of any
...
/-- **Proposition 6 (i)**. `v̂(μ*_C) = v̄(μ*_C)` for every non-empty
...
theorem tent_eq_at_faceMax (F : Γ.FaceMax) (hT : Γ.TentAssumption F)
  ...
  have hcombo : Γ.chainCombo F ch (fun j => if j = j0 then 1 else 0) = F.μstar C hne hsub := by
    unfold CPDLinear.RichGame.chainCombo; simp +decide [ hj0 ] ;
  have := tent_chainCombo F hT ch ( fun j => if j = j0 then 1 else 0 ) ( fun j => by positivity ) ( by simp +decide ) ; aesop;

/-- **Proposition 6 (ii)**. Along any nested chain the tent is affine in the
weight vector. -/
...
/-- **Proposition 6 (iii)**. Any `f` satisfying (i) and (ii) equals the tent
on `ΔΘ`. -/
