...
/-!
# Betweenness + genericity theorem (working-paper appendix)

**Working-paper theorem (plain betweenness + genericity ⇒ existence).** The
genericity analogue of Theorem 2 and its B*-COE corollary: with `V`
single-valued, `v̄` satisfying plain betweenness (B), and `G` generic
(Definition 11), the entire greedy theory (Algorithm 3) goes through with the
B*-only no-merge-up step (`btw_merging_impossible`) replaced by the
three-line generic bound (Lemma 7 `btw_merging` + Lemma 5 `value_id` +
`Generic` injectivity).
...
/-! ### Working-paper theorem (betweenness + genericity) -/

/-- **Working-paper theorem (i), no-halt form.** Under single-valued `V`, B,
...
/-- **Working-paper theorem (i), existence.** Under single-valued `V`, B, and
...
/-- **Working-paper theorem (ii).** Under single-valued `V`, B, and
...
/-- **Working-paper theorem (iii).** Under single-valued `V`, B, and
