...
/-!
# Feasible beliefs and skeptical payoffs  (writeup §3)

The **set of feasible beliefs** `𝓕(m) := {μ_σ(·|m) | σ a sender strategy,
m ∈ X(σ)}` (Def set-feasible-beliefs).  Lemma feasible-polytope: `𝓕(m)` equals
the polytope cut out by (i)–(iii), and is a non-empty compact convex polytope.
The **skeptical payoff** `u̲(θ) := max_{m∈M(θ)} min_{μ∈𝓕(m)} v̲(μ)`
(Def skeptical-payoff), well defined.
-/
...
/-- **The set of feasible beliefs** `𝓕(m)`. -/
def feasibleBeliefs (m : Msg) : Set (T → ℝ) :=
  {μ | ∃ s : Strategy G, m ∈ s.evidence ∧ s.belief m = μ}

/-- The feasible-belief polytope cut out by conditions (i)–(iii). -/
...
/-- **Lemma feasible-polytope (characterization (i)–(iii)).** -/
...
/-
**Skeptical payoffs are well-defined**: the outer max and inner min are
attained, with `u̲(θ) = v̲(μ)` for some `m ∈ M(θ)`, `μ ∈ 𝓕(m)`.
-/
