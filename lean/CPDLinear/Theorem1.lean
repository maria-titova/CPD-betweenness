...
/-!
# Theorem 1 — existence, no-halt under QC*, essential uniqueness under genericity
  (writeup §6, `thm:one`)

If the message mapping is complete (M-C) and `v̄` is quasiconcave (QC), a
coalition-proof PBE exists.  If in addition `v̄` is strictly quasiconcave (QC*),
no run of the greedy algorithm halts and every greedy partition is a
coalition-proof PBE partition.  If the game is generic, the coalition-proof PBE
partition is essentially unique (same length, cells, and payoffs).
-/
...
/-- **Theorem 1, existence.** Under M-C and QC a coalition-proof PBE partition
(equivalently, a coalition-proof PBE) exists. -/
...
/-- **Theorem 1(i), no halt.** Under M-C and QC*, a greedy partition exists (the
greedy algorithm never halts), and every greedy partition is a coalition-proof
PBE partition. -/
...
/-- **Theorem 1(ii), essential uniqueness.** Under M-C, QC and genericity, any two
